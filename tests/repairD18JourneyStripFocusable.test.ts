...
// horizontally-scrollable element is the featured campaign-lesson row.
//
// Updated again for the homepage marquee change: that row's overflow-x-auto container moved out of
// app/page.tsx into its own client component (components/lessons/CampaignLessonsMarquee.tsx, which
// needs client-side state for the auto-scroll/pause behavior that a server-component page can't
// hold). Same underlying guarantee as before (no scrollable content without keyboard access),
// checked against wherever the element currently lives.
...
test("D18: every horizontally-scrollable overflow-x-auto container on the homepage is keyboard-focusable", () => {
  const source = read("components/lessons/CampaignLessonsMarquee.tsx");
  const matches = [...source.matchAll(/<div\s[^>]*className="[^"]*overflow-x-auto[^"]*"([^>]*)>/g)];
  assert.ok(matches.length > 0, "expected at least one horizontally-scrollable container in the homepage's campaign-lesson row");
