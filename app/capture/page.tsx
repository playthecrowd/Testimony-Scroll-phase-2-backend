...
export default function CapturePage() {
  ...
  const [created, setCreated] = useState<{ slug: string; title: string; churchSlug: string } | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.lesson) setCreated(result.lesson);
  }
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-muted text-sm mb-8">
          &ldquo;{created.title}&rdquo; was saved as a draft. Open it to preview and publish it to the Lessons
          Library and your church archive.
        </p>
        <div className="flex flex-wrap justify-center gap-3">
          <LinkButton href={`/lessons/${created.slug}`}>View Draft &amp; Publish</LinkButton>
          <LinkButton href={`/churches/${created.churchSlug}`} variant="secondary">
            View Church Archive
          </LinkButton>
          <button
            onClick={() => {
    ...
    <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-6 md:py-8">
      ...
      {churchesLoading ? (
        ...
        <div className="qk-card p-10 text-center text-muted text-sm">
          Complete your church setup before capturing a lesson.{" "}
          <Link href="/onboarding/church" className="text-accent-blue-light hover:underline">
            Set up your church
          </Link>
        </div>
      ) : (
