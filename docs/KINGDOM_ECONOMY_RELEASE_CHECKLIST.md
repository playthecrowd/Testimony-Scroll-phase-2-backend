...
| 2 | Migration state | ✅ Verified, with 2 pending | `0001`–`0033` live and matching local/remote. `0034` (trigger rename) and `0035` (reconciliation diagnostic) are written, dry-run-verified, and **await explicit push authorization** — see item 3. |
| 3 | Database backup recommendation | ⚠️ Owner action required | Before pushing `0034`/`0035` (or any future migration), take a manual Supabase backup/snapshot of the linked project if one isn't already covered by Supabase's own automatic backup schedule for this plan tier. Both pending migrations are low-risk (a trigger rename, a new read-only function) but this is a standing recommendation for *any* production database change, not specific to these two. |
| 4 | Supabase verification | ✅ Verified | All 10 economy/progression tables, both leaderboard views, all 16 public RPCs + 4 private helpers, all 7 triggers, RLS on all 10 tables, correct grants — all confirmed live (`docs/PHASE11_5_AUDIT.md` §18). |
...
| 18 | Deployment approval gate | ⚠️ Awaiting explicit approval | Per standing project convention: no merge to `main`, no Vercel deployment, and no push of migrations `0034`/`0035` happens without the owner's explicit, separate go-ahead for each. This checklist itself does not constitute that approval. |
...
passes. The items marked ⚠️ above are not gaps in this phase's work — they are the specific,
honestly-named set of things that can only be verified by the owner, in an environment this
sandbox does not have access to (a real Vercel project, real Supabase environment variables, real
test accounts, a real browser session). Release readiness from this phase's own scope is
**conditional on those owner-side items**, not blocked by any known code or database defect.
